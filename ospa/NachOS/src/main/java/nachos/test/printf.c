...
static char
digittoascii(unsigned n, int uppercase)
{
  assert(n < 36);
...
static int
mcharprint(char** s, char* chars, int length)
{
...
static int
integerprint(char** s, int n, unsigned base, int min, int zpad, int upper)
{
  char buf[32];
...
static int
stringprint(char** s, char* string)
{
  return mcharprint(s, string, strlen(string));
}

static int
_vsprintf(char* s, char* format, va_list ap)
{
...
void
vsprintf(char* s, char* format, va_list ap)
{
...
void
vfprintf(int fd, char* format, va_list ap)
{
