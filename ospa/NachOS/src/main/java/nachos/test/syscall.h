 ...
 * reuse.
 *
 * Note that, if any processes still have the file open, the file will
 * remain in existence until the last file descriptor referring to it
 * is closed (the underlying file system implementation will handle
 * this detail). However, creat() and open() will not be able to
