...
public class SIM extends Applet {
  ...
  // ***** Public Class data *****
  // --------------------------------------------------------------------
  public static SIM TheSIM;
  public static SOSData sosData;
  public static HWSimulation hw;
  ...
  private final DefaultListModel proc0_model;
  private final JList proc0_trace;
  private final JScrollPane proc0_scroll;
  ...
  public SIM() {
    ...
    if ((trace_mask & TraceApp) == TraceApp) cb_app.setSelected(true);
    cb_app.addItemListener(
        new ItemListener() {
          public void itemStateChanged(ItemEvent e) {
            if (e.getStateChange() == ItemEvent.SELECTED) trace_mask |= TraceApp;
            else trace_mask &= ~TraceApp;
          }
        });
    trace_options.add(cb_app);

    cb_hw = new JCheckBox("HW");
    if ((trace_mask & TraceHW) == TraceHW) cb_hw.setSelected(true);
    cb_hw.addItemListener(
        new ItemListener() {
          public void itemStateChanged(ItemEvent e) {
            if (e.getStateChange() == ItemEvent.SELECTED) trace_mask |= TraceHW;
            else trace_mask &= ~TraceHW;
          }
        });
    trace_options.add(cb_hw);

    cb_sim = new JCheckBox("SIM");
    if ((trace_mask & TraceSIM) == TraceSIM) cb_sim.setSelected(true);
    cb_sim.addItemListener(
        new ItemListener() {
          public void itemStateChanged(ItemEvent e) {
            if (e.getStateChange() == ItemEvent.SELECTED) trace_mask |= TraceSIM;
            else trace_mask &= ~TraceSIM;
          }
        });
    trace_options.add(cb_sim);

    cb_syscall = new JCheckBox("Syscall");
    if ((trace_mask & TraceSOSSyscall) == TraceSOSSyscall) cb_syscall.setSelected(true);
    cb_syscall.addItemListener(
        new ItemListener() {
          public void itemStateChanged(ItemEvent e) {
            if (e.getStateChange() == ItemEvent.SELECTED) trace_mask |= TraceSOSSyscall;
            else trace_mask &= ~TraceSOSSyscall;
          }
        });
    trace_options.add(cb_syscall);

    cb_pm = new JCheckBox("PM");
    if ((trace_mask & TraceSOSPM) == TraceSOSPM) cb_pm.setSelected(true);
    cb_app.addItemListener(
        new ItemListener() {
          public void itemStateChanged(ItemEvent e) {
            if (e.getStateChange() == ItemEvent.SELECTED) trace_mask |= TraceSOSPM;
            else trace_mask &= ~TraceSOSPM;
          }
        });
    trace_options.add(cb_pm);

    cb_disk = new JCheckBox("Disk");
    if ((trace_mask & TraceSOSDisk) == TraceSOSDisk) cb_disk.setSelected(true);
    cb_app.addItemListener(
        new ItemListener() {
          public void itemStateChanged(ItemEvent e) {
            if (e.getStateChange() == ItemEvent.SELECTED) trace_mask |= TraceSOSDisk;
            else trace_mask &= ~TraceSOSDisk;
          }
        });
        ...
        new ActionListener() {
          public void actionPerformed(ActionEvent e) {
            if (sosStarted) {
              ...
              SIM.Trace(TraceAlways, "SOS already started. You can't start it twice!");
            } else {
        ...
        new ActionListener() {
          public void actionPerformed(ActionEvent e) {
            if (!sosPauser.isPaused()) {
              ...
              sosPauser.pause();
              HWSimulation.theDiskThread.suspend();
              HWSimulation.hwTimerThread.suspend();
            }
          }
        });
    ...
    bresume.addActionListener(
        new ActionListener() {
          public void actionPerformed(ActionEvent e) {
            if (sosPauser.isPaused()) {
              SIM.Trace(TraceAlways, "Resuming SOS");
              sosPauser.unpause();
              HWSimulation.theDiskThread.resume();
              HWSimulation.hwTimerThread.resume();
            }
          }
        });
    ...
    bexit.addActionListener(
        new ActionListener() {
          public void actionPerformed(ActionEvent e) {
            if (isApplet) {
              Trace(TraceAlways, "*** Unload the page to exit the applet ***");
            } else {
              System.exit(0);
            }
          }
  ...
  // ***** Trace function *****
  // --------------------------------------------------------------------
  public static void Trace(int trace_type, String msg) {
    if ((trace_type & SIM.TheSIM.trace_mask) == 0)
      // we are not recording this type of trace message
      return;
    TheSIM.TraceMessage(
        Thread.currentThread().toString().substring(7)
            + "@"
            + (System.currentTimeMillis() - SIM.TheSIM.startTime)
            + ": "
            + msg);
    if (!SIM.TheSIM.isApplet) SIM.TheSIM.pout.println(msg);
    String msg1 = "@" + (System.currentTimeMillis() - SIM.TheSIM.startTime) + ": " + msg;
    int cur_proc = SIM.sosData.current_process;
    if (cur_proc > 0) SIM.hw.ProcessTrace[cur_proc].addElement(msg1);
    else SIM.TheSIM.proc0_model.addElement(msg1);
  }
  ...
  public void init() {
    ...
    Trace(TraceAlways, "Java version is " + System.getProperty("java.version"));
    Trace(TraceAlways, "Java vendor is " + System.getProperty("java.vendor"));
    if (!isApplet) {
  ...
  public synchronized void TraceMessage(String msg) {
