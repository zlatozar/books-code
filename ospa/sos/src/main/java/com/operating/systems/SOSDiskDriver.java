...
class SOSDiskDriver {
  ...
  void DiskIO(int command, int disk_block, int buffer_address) {
    SIM.Trace(
        SIM.TraceSOSDisk,
        "Queue Disk IO request: block=" + disk_block + ", buffer=" + buffer_address);
    ...
    req.buffer_address = buffer_address + SIM.sosData.pd[cur_proc].base_register;
    req.pid = cur_proc;
  ...
  void IssueDiskRead(int block_number, int buffer_address, int enable_disk_interrupt) {
    SIM.Trace(
        SIM.TraceSOSDisk,
        "Disk Read -- block=" + block_number + ", buffer_address=" + buffer_address);
    SIM.hw.SetCellUnmapped(SIM.DiskAddressRegister, buffer_address);
    SIM.hw.SetCellUnmapped(
        SIM.DiskControlRegister, (enable_disk_interrupt << 30) + (block_number << 10));
  }
  ...
  void IssueDiskWrite(int block_number, int buffer_address, int enable_disk_interrupt) {
    SIM.Trace(
        SIM.TraceSOSDisk,
        "Disk Write -- block=" + block_number + ", buffer_address=" + buffer_address);
    SIM.hw.SetCellUnmapped(SIM.DiskAddressRegister, buffer_address);
    SIM.hw.SetCellUnmapped(
        SIM.DiskControlRegister, (1 << 31) + (enable_disk_interrupt << 30) + (block_number << 10));
  }
}
