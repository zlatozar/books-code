...
class SOSProcessManager {
  ...
  public static void SaveProcessState(int pid) {
  ...
  int CreateProcessSysProc(int app_number, int arg) {
    ...
    for (pid = 1; pid < NumberOfProcesses; ++pid) {
      if (!(SIM.sosData.pd[pid].slotAllocated)) break;
    }
  ...
  void RunProcess(int pid) {
    ...
    if (pid >= 0) {
      ...
      SIM.Trace(
          SIM.TraceSOSPM, "Dispatching pid " + pid + " :" + SIM.hw.ProcessThread[pid].toString());
      // Set up the memory mapping registers for the process
      HWSimulation.MemoryBaseRegister = SIM.sosData.pd[pid].base_register;
      HWSimulation.MemoryLimitRegister = SIM.sosData.pd[pid].limit_register;
