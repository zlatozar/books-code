...
class SOSStart implements Runnable {
  ...
  public void run() {
    ...
    SIM.hw.SetCellUnmapped(SIM.SyscallIntVector, new SOSSyscallIntHandler());
    SIM.hw.SetCellUnmapped(SIM.TimerIntVector, new SOSTimerIntHandler());
    SIM.hw.SetCellUnmapped(SIM.DiskIntIntVector, new SOSDiskIntHandler());
    SIM.hw.SetCellUnmapped(SIM.ProgErrIntIntVector, new SOSProgErrIntHandler());
  ...
  private void InitializeProcessSystem() {
    ...
    for (i = 0; i < (SOSProcessManager.NumberOfMessageBuffers - 1); ++i)
      ...
      SIM.hw.SetCellUnmapped(
          SIM.MessageBufferArea + SOSProcessManager.MessageSize * i, new Integer(i + 1));

    // The last message buffer in the free chain has a marker so
    // you can tell you are at the end of the list.
    SIM.hw.SetCellUnmapped(
        SIM.MessageBufferArea
            + SOSProcessManager.MessageSize * (SOSProcessManager.NumberOfMessageBuffers - 1),
        new Integer(-1));
