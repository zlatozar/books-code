...
class Machine {
    ...
    final static int SCST = 0;
    final static int SVAR = 1;
    final static int SADD = 2;
    final static int SSUB = 3;
    final static int SMUL = 4;
    final static int SPOP = 5;
    final static int SSWAP = 6;

    public static void main(String[] args) {

        final int[] rpn1 = {SCST, 17, SVAR, 0, SVAR, 1, SADD, SSWAP, SPOP};
        System.out.println("Evaluate: {SCST, 17, SVAR, 0, SVAR, 1, SADD, SSWAP, SPOP}");
        System.out.println(seval(rpn1));

        System.out.println("\n");

        final int[] rpn2 = {SCST, 17, SCST, 22, SCST, 100, SVAR, 1, SMUL, SSWAP, SPOP, SVAR, 1, SADD, SSWAP, SPOP};
        System.out.println("Evaluate: {SCST, 17, SCST, 22, SCST, 100, SVAR, 1, SMUL, SSWAP, SPOP, SVAR, 1, SADD, SSWAP, SPOP}");
    ...
    static int seval(int[] code) {
        ...
        while (pc < code.length)
            ...
            switch (instr = code[pc++]) {
                ...
                default:
                    throw new RuntimeException("Illegal instruction " + instr + " at address " + (pc - 1));
            }
