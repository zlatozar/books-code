...
(* NOTE: A lot of code is duplicate from ch02/Intcomp1/Incomp.fs *)

(* Simple arithmetic expressions. Same as in Chapter 2 *)
...
let rec private fmt1 (e: Expr) :string =
    ...
    | Var x  -> x
    | Let(x, erhs, ebody) ->
        String.concat " " ["let"; x; "="; fmt1 erhs; "in"; fmt1 ebody; "end"]
    | Prim(ope, e1, e2) ->
...
// New version. See how is implemented in Chapter 2
...
let rec reval (inss: RInstr list) stack =
    ...
    | (ins0 :: rest), _ ->      // new (compare with Chapter 2 implementation)
...
let rec rcomp (e: Expr) :RInstr list =   // Chapter 2 version is better
    match e with
    | CstI i -> [RCstI i]
...
// We can store intermediate results as well as variable bindings on the same stack.  This is
// possible because the scopes of variables are statically nested: the scope of a variable
// (and hence the need for its binding) does not extend beyond the expression in which it is
// bound.
...
// To compile for the single-stack machine, we must keep count (at compile-time) of the
// intermediate results, interspersed between the variable bindings on the stack.  For this
// we need a compile-time variable environment with dummies Intrm for intermediate values.

type RTvalue =
  | Intrm             (* An intermediate result or marker to calculate offset *)
...
// Net effect principle: when c = scomp e {} is the code generated for expression e, then the
// net result of evaluating c is to push the value of e onto the evaluation stack.
//
// Hence if v = eval e [] [] then for any stack stk, seval (scomp e []) s equals v :: s
