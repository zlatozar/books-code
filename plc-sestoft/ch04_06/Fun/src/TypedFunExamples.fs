...
(*
   F# will not infer the same type as above (the variables in p1, p2, p3, p4 will not be
   generalized) because of the so-called _value restriction_ on polymorphism, which ensures
   type system soundness in the presence of destructive update.  An F# example similar to
   the above can be obtained by eta-expansion: turning let f = e into let f p = e p which
   is valid when f is a function.  The type of p5 below involves 64 different type
   variables:
