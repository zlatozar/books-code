(*
   Interpreter for micro-C, a fraction of the C language
     ...
     - A value is an integer; it may represent an integer or a pointer, where a pointer is
   just an address in the store (of a variable or pointer or the base address of an
   array).
   ...
   with the way array-type parameters are handled in C (and the way that array-type
...
// Extend local variable environment so it maps x to nextloc (the next store location) and
// set store[nextloc] = v.
...
// Execute statement and produce an updated store
...
// Execute statement or declaration and produce updated local
...
and eval e locEnv gloEnv store :int * store =
    ...
    | Andalso(e1, e2) ->
        ...
        if i1<>0
          then eval e2 locEnv gloEnv store1
          else res
    | Orelse(e1, e2) ->
        let (i1, store1) as res = eval e1 locEnv gloEnv store
        if i1<>0
